...
public class Veranstaltungsanmeldung extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String strSelect = 	"SELECT INS_VERANSTALTUNG.ID, KON_VERANSTALTUNG.NAME, DOZENT.VORNAME, DOZENT.NACHNAME, INS_VERANSTALTUNG.LVNR, INS_VERANSTALTUNG.SEMESTER,  KON_VERANSTALTUNG.ART, KON_VERANSTALTUNG.SWS, KON_VERANSTALTUNG.MINTEILNEHMER,  KON_VERANSTALTUNG.ERWTEILNEHMER\n" + 
								...
								"AND INS_VERANSTALTUNG.ID NOT IN (\n" + 
			...
			sb.append(HtmlBuilder.createDocumentHeader("Veranstaltungsanmeldung - FlexVZ")); 
			sb.append(
					"<h2>Veranstaltungsanmeldung</h2>\n" + 
					"<p>Wähle einen Eintrag aus der folgenden Tabelle, für den du dich anmelden möchtest:</p></br>\n" + 
					"<form action=\"./Veranstaltungsanmeldung/confirmation\" method=\"get\">\n");
			...
			if (rset.first()) {
				sb.append(
					HtmlBuilder.createSubmitButton("Veranstaltungsteilnahme anmelden"));
			}
			sb.append(
					"</form>\n" +
					"</br>\n" +
